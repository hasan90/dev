...
//include_once(WEBSITE_URL.'platform/framework/mailSender.php');
...
$con = mysql_connect(DB_HOST,DB_USERNAME,DB_USER_PASSWORD);
	...
	$selectdb = mysql_select_db(DB_DBNAME,$con);
...
$inqury_id=trim($_REQUEST['inqury_id']);


$update_qry="update inquiry set call_duration ='".$duration."',connection_duration ='".$duration."',call_start_time='".$start_time."',call_end_time='".$end_time."',inquiry_type='".$status. "', statucallbackdata ='".print_r($_REQUEST,true)."', twilio_sid='".$twilio_sid."'  where id='".$inqury_id."'";
		//echo $update_qry;
...
//$query="SELECT id,twilio_sid FROM inquiry WHERE twilio_sid != '' and ((call_start_time='' and call_end_time='') or (call_start_time IS NULL and call_end_time IS NULL)) limit 0,100"; "SELECT * FROM inquiry WHERE twilio_sid='".$twilio_sid."'"
$query="SELECT * FROM inquiry WHERE twilio_sid='".$twilio_sid."'";
...
if($cnt>0)
{
    ...
    if(mysql_num_rows($mailResult) > 0)
    {
        ...
        if($time == '0:00')
         ...
         if($_REQUEST['check_for_sendeing_mail'] == 1 && !empty($_REQUEST['notification_email']))
         { 
		           $fName = ""; 
		         if(!empty($first_name))
...
*/
                ...
                $status =  MailSender::sendMail($_REQUEST['notification_email'], $subject, $body);
