...
//                        die();
			...
			{		
				...
				$this->connecttotwilio(md5($time),$last_insert_id,$form_id,$user_phone,$notification_email,$dataOfPostCallDetail);
 ...
 public function connecttotwilio($ression ,$last_insert_id,$form_id,$user_phone,$email,$dataOfPostCallDetail)
    {
    	 $filter=new Zend_Filter_StripTags();
...
//               die();
               ...
               else
               {
                        ...
                        $inquiryTable->update($inquiry_update_data,'id='.$id);
               			...
               			if($check_email_send == 1 && !empty($email_send))
	                        {
	                        	$inquiry = new inquiry();
	               			 	$inquiry=$inquiry->fetchRow($inquiry->select()->where('id='.$id));
	                        	$call_duration = $inquiry['call_duration'];
	                        	 
	               			 	$mail_body ="First Name is : $first_name <br/><br/>Last Name is : $last_name <br/><br/> Call Duration :  $call_duration ";
	               				 
	                        	$mail = new Zend_Mail();
	
		                        $mail->setBodyHtml($mail_body);
		                        $mail->setFrom(SITE_SUPPORT_EMAIL, WEBSITE_NAME);
		                        $mail->addTo($email_send, 'Admin');
		                        $mail->setSubject('Your Form@ctive Call Detail');
		                        $mail->send();
	                        }
