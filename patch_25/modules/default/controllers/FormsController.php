...
class FormsController extends BaseController
{
    ...
    public function indexAction()
    {
   		 ...
   		 if(!empty($session->member_id) && $session->user_type == 1)
		{
			...
			$session->admin_id = $session->member_id;
			$session->member_id = $customer_id;
			$this->view->loggedin_customer_id = $session->admin_id;
		}
    ...
    public function validatePhoneNumberAction()
    {
        ...
        $formats = array(
				...
				$newnumber = str_replace($order, $replace, $number);

        $number = trim(preg_replace('/[0-9]/', '#', $newnumber));
        ...
        if (in_array($number, $formats)) {
            ...
            //validate user phone number
            	...
            	$call_delay = $formsTableData['call_delay'];
            	
            	if($number_type == 'home_phone_status')
            		$extension = trim($formsTableData['home_phone_extension']);
            		
            	if($number_type == 'business_phone_status')	
                	$extension = trim($formsTableData['business_phone_extension']);
            ...
            $response = $client->request("/$ApiVersion/Accounts/$AccountSid/OutgoingCallerIds",
            	...
            	"Extension" => $extension,
            	"StatusCallback" => WEBSITE_URL."updateConnectionPhoneNumbersVeryficationStatus.php?form_id={$form_id}&number_type={$number_type}&phone_type={$phone_type}&customerId={$customerId}&number={$newnumber}"
            ));
    ...
    //       print_r($response);
             ...
             if($response->ErrorMessage == 'Phone number is already verified.')
             {
				...
				$forms_data = array();
             	...
             	$validateNumbers = new validateNumbers();
	         	$validate_numbers=$validateNumbers->fetchRow($validateNumbers->select()->where("numbers='$newnumber'"));
	         	
				if(count($validate_numbers) == 0)
				{
					$validateNumbers_num['numbers']=$newnumber;
					$validateNumbers->insert($validateNumbers_num);
				}
             }
        ...
        if (in_array($number, $formats) && !empty($form_id))
         {
			...
			$forms_data=$formTable->fetchRow($formTable->select()->where("customer_id=".$session->member_id." and id=".$form_id));
         	...
         	{
         		if($number_type == "home_phone_status")
					$response = $forms_data['business_phone_validated']."@@"."1";
					
				if($number_type == "business_phone_status")
					$response = "1"."@@".$forms_data['home_phone_validated'];	
    ...
    {         
		...
		{                        
			...
			$this->view->include_form_name_on_form = $forms_data['include_form_name_on_form'];
			$this->view->toRepeatTheAnnouncement =  trim($forms_data['to_repeat_the_announcement']);
			$this->view->callDelay =  trim($forms_data['call_delay']);
			$this->view->business_phone_extension = trim($forms_data['business_phone_extension']);
			$this->view->home_phone_extension = trim($forms_data['home_phone_extension']);
			$flags = trim($forms_data['flags']);
			$this->view->includeImage =  substr(trim($forms_data['flags']),0,1);
			$this->view->form_type = $forms_data['form_type'];
		...
		if($this->_request->isPost())
		{
			...
			$caller_id = $filter->filter($this->_request->getPost('caller_id'));
		 	...
		 	$include_form_name_on_form = $filter->filter($this->_request->getPost('includeFormName'));
		 	$toRepeatTheAnnouncement = $filter->filter($this->_request->getPost('toRepeatTheAnnouncement'));
		 	$callDelay = $filter->filter($this->_request->getPost('callDelay'));
		 	$business_phone_extension = $filter->filter($this->_request->getPost('business_phone_extension'));
		 	$home_phone_extension = $filter->filter($this->_request->getPost('home_phone_extension'));
		 	
		 	$includeImage = trim($filter->filter($this->_request->getPost('includeImage')));
		 	
			$myFile = "testFile7.txt";
			$fh = fopen($myFile, 'w') or die("can't open file");
			$stringData = "business_phone_extension=>".$business_phone_extension."\n home_phone_extension=>".$home_phone_extension;
		...
		/******************** End => when the form type will be changed  **************/		
			...
			$form_data['call_delay'] = trim($callDelay);
			$form_data['business_phone_extension'] = trim($business_phone_extension);
			$form_data['home_phone_extension'] = trim($home_phone_extension);
			$form_data['status']=$status;
			$form_data['business_phone'] = $business_phone;
			$form_data['home_phone']= $home_phone; 
			$form_data['form_type'] = $form_type; 
			
			if($includeImage == 1 || $includeImage == '')
				$form_data['flags'] = $this->setFlagBit($flags,1,1);
			else
			 	$form_data['flags'] = $this->setFlagBit($flags,1,0);
			 		 
			$form_data['include_form_name_on_form'] = $include_form_name_on_form; 
   ...
   // Delete customer.
	public function deleteAction()
	{
            if($this->getRequest()->getParam('id'))
            {
               ...
               $session = new Zend_Session_Namespace('Zend_Auth');	 
                ...
                if(empty($cid))
                	$cid = $session->member_id; 
                
                $this->_redirect('/forms/index/customer_id/'.$cid);
                exit;
            }
	}
     ...
     */
		...
		//	 $this->view->data = $emailnotifications_data;	
				if(count($emailnotifications_data) > 0)
				{
					 ...
					 $this->view->subject_line_of_email = $emailnotifications_data['subject_line_of_email'];
					 $flags = trim($emailnotifications_data['flags']);
					 $this->view->includeImage =  substr(trim($emailnotifications_data['flags']),0,1);
					 $this->view->sendEmailForCallDuration =  substr(trim($emailnotifications_data['flags']),1,1);
					 $this->view->email_address_to_use_in_from_field_to_prospective_lead =$emailnotifications_data['email_address_to_use_in_from_field_to_prospective_lead'];
					 $this->view->email_notification_id=$emailnotifications_data['id'];			
				} 
				else
				{
					  $this->view->answered_calls =  1;
					 $this->view->connections =  1;
				}
			 }
		...
		if($this->_request->isPost())
		{
			...
			$email_address_to_use_in_from_field_to_prospective_lead = trim($filter->filter($this->_request->getPost('email_address_to_use_in_from_field_to_prospective_lead')));	
			...
			$form_id = $filter->filter($this->_request->getPost('form_id'));	
			$subject_line_of_email = $filter->filter($this->_request->getPost('subject_line_of_email'));

			
			
			$includeImage = trim($filter->filter($this->_request->getPost('includeImage')));
			$sendEmailForCallDuration = trim($filter->filter($this->_request->getPost('sendEmailForCallDuration')));
		...
		if( empty($incomplete_calls) && empty($after_hour_calls) && empty($unanswered_calls) && empty($answered_calls) && empty($connections) && empty($connections) && empty($send_email_notification_pros_leads))
			...
			else
			{
					...
					if($includeImage == 1 || $includeImage == '')
						$notification_data['flags'] = $this->setFlagBit($flags,1,1);
					else
					 	$notification_data['flags'] = $this->setFlagBit($flags,1,0);
					 	
					 	
					if($sendEmailForCallDuration == 1 || $sendEmailForCallDuration == '')
						$notification_data['flags'] = $this->setFlagBit($notification_data['flags'],2,0);
					else
					 	$notification_data['flags'] = $this->setFlagBit($notification_data['flags'],2,1);
					...
					$notification_data['subject_line_of_email'] =  trim($subject_line_of_email);		 					
					$notification_data['send_email_notification_pros_leads'] =trim($send_email_notification_pros_leads); 
					$notification_data['email_address_to_use_in_from_field_to_prospective_lead'] =trim($email_address_to_use_in_from_field_to_prospective_lead); 
    		...
    		if($this->_request->isPost())
                {
						...
						$findme = "http";
						$pos = strpos($redirect_url, $findme);
						 
						 if($pos === false) 
						    $redirect_url = $findme."://".$redirect_url;
...
/**
    ...
    public function getformAction()
    {
        ...
        $this->view->formname=$forms_data['form_name'];
        $this->view->include_form_name_on_form = $forms_data['include_form_name_on_form'];
        $this->view->include_formActivate_image = substr(trim($forms_data['flags']),0,1);
		...
		{		 
		 ...
		 if(count($forms_data)>0){			 	
			...
			$redirect_type=$forms_data['redirect_type'];
		 	...
		 	$caller_id=trim($forms_data['caller_id']);
		  ...
		  /* NOTE THESE RULES ARE HARD CODED IN THE ADMIN PANEL, IT WILL BE MANAGED BY THE ADMIN*/
                       ...
                       if(($start_time <= $customer_current_time)&& ($customer_current_time <= $end_time))
                       {
                      	 ...
                      	 // SET THE PHONE NUMBER FOR CALLING
                              ...
                              $extension = $forms_data['business_phone_extension'];
                       }
                       else
                       {
                       			$phone = "";
                              ...
                              $extension = $forms_data['home_phone_extension'];     
                       }
				   //echo $phone.	'------1500';
		  }		  
		 ...
		 /* CHECKING CUSTOMER RULES FOR SENDING CALL END HERE */
			...
			$lastname=trim($this->_request->getPost('standard2'));
		 	...
		 	$customerHeader .="Thank you for your interest in ".$form_owner_data['companyname'].".  We have received your information and will be in touch shortly. Please keep a copy of this email for your reference.<br><br>Sincerely, <br/>".$form_owner_data['companyname']."<br><br>";
			...
			if(trim($string_to_pass_to_twilio)!='')
				...
				if(!empty($homephone))
                      ...
                      /* Start For randamly created caller_id according to doc bugs when selected caller_id as "Active Connection Number" updated by Hasan saeed*/    
							
                                  /* if($caller_id == 0) 
                                   {
                                   		$forms_update_data = array();
                                   		$forms = new Forms();
                                   		$forms_update_data['caller_id']=trim($phone);
										$forms->update($forms_update_data,'id='.$form_id);
										$caller_id = trim($phone);
                                   }*/
                                    
                      /* End For randamly created caller_id according to doc bugs when selected caller_id as "Active Connection Number" updated by Hasan saeed*/               
...
//                        die();
					...
					if(trim($emailnotification_data['send_email_notification_pros_leads'])=='yes' && trim($email) != '' && !empty($email))
		             {
                        $mail = new Zend_Mail();
						
                       			 $image = "";  
                        		$subject = "";        
                        	    $subject = trim($emailnotification_data['subject_line_of_email']);  
                        	    if(empty($subject) || $subject == null)
                        	    	$subject = "Thank you for your interest";  
                        	    if(substr($emailnotification_data['flags'],0,1) == 0)
                                ...
                                $mailBody = $customerHeader."</div></div>";
                                $mail->setBodyHtml($mailBody);
                                $mail->setFrom($form_owner_data['email'], $form_owner_data['companyname']);
                                
                                $mail->addTo(trim($email),$firstname.' '.$lastname);                                
                        ...
                        if(trim($notification_email) != '' && !empty($notification_email))
                        {	
                                ...
                                {
                                	$mail->addTo(trim($emails[$i]),'FormActivate Notification Recipient');
                                	$isEmailsExists++;
	                            }
	                            if($isEmailsExists > 0)
	                          	  	$mail->send();
...
//                        die();
			...
			$dataOfPostCallDetail = substr($emailnotification_data['flags'],1,1)."-".trim($emailnotification_data['email_address_to_use_in_from_field_to_prospective_lead'])."-".$extension;
			
			if($connecttotwilio==1)
			{				
				$this->_redirect('/forms/connecttotwilio/rsession/'.md5($time).'/id/'.$last_insert_id.'/form_id/'.$form_id.'/user_phone/'.$user_phone.'/notification_email/'.$notification_email.'/dataOfPostCallDetail/'.$dataOfPostCallDetail);	exit;
			}
...
//                            $this->_redirect('/forms/thanks');   exit;
    ...
    public function connecttotwilioAction()
    {
    	 ...
    	 $dataOfPostCallDetail = $this->_request->getParam('dataOfPostCallDetail');
    	 $post_call_detail_data = explode('-',$dataOfPostCallDetail);
         
         $check_for_sendeing_mail = $post_call_detail_data[0];
         $notification_email = $post_call_detail_data[1];
    	  $extension = $post_call_detail_data[2];
                ...
                /* Initiate a new outbound call by POST'ing to the Calls resource */
              	 $response = $client->request("/$ApiVersion/Accounts/$AccountSid/Calls",
                    "POST", array(
                        ...
                        "StatusCallback" => WEBSITE_URL."twilio_call_time_update.php?notification_email={$notification_email}&customer_id={$customer_id}&check_for_sendeing_mail={$check_for_sendeing_mail}&inqury_id={$id}"
                    ));
