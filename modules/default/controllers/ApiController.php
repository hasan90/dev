...
class ApiController extends Zend_Rest_Controller
{
    ...
    public function responsereturn($inquiryId,$auth_key)
    {
		...
		if(strpos(trim($authKey_format[1]),'.') > 0)
		{
			
			/*$myFile = "testFile7.txt";
					$fh = fopen($myFile, 'w') or die("can't open file");
					$stringData = "in=>".$authKey_format[1];
		...
		{
			 	if(count($inquiry_data) == 1 && ($authKey_format[1] == "json" || $authKey_format[1] == "xml" || $authKey_format[1] == 3 || $authKey_format[1] == 4))
			 	{
			    	...
			    	$test_array = array (
						...
						if($authKey_format[1] == "json" || $authKey_format[1] == 3)
						{				
							header('Content-type: application/json');
							echo json_encode($test_array );
						}
						if($authKey_format[1] == "xml" || $authKey_format[1] == '' || $authKey_format[1] == "default" || $authKey_format[1] == 4)
						{	
							$xml = RestUtils::array_to_xml($test_array, new SimpleXMLElement('<response/>'))->asXML();
							RestUtils::sendResponse(200, $xml, 'application/xml');
						}
			 	}
			 	else if($authKey_format[1] == "default" || trim($authKey_format[1]) != '' || $authKey_format[1] == 1)
				 {
    ...
    public function putAction()
    {
			...
			$format = trim($this->_request->getParam('response'));
		 	if(empty($format) || trim($format) == '')
		 	{
			 	$status = 1;
			 	$formTable = new Forms();
		        $data=$formTable->fetchRow($formTable->select()->where("id='".$form_id."' and status='".$status."'"));
		        $format = $data['redirect_type'];
		 	}
		 	
		 	
   	 		if($format != 1 && (empty($api_key) || empty($member_id) || empty($form_id)))
       		{
       		$test_array = array (
		 ...
		 /********* end validation of Required fields *******/
			...
			if(trim($this->_request->getParam('phonenumber')) != '') 
			{
				...
				$newnumber = str_replace($order, $replace, trim($this->_request->getParam('phonenumber')));
       		 ...
       		 $number = trim(preg_replace('/[0-9]/', '#', $newnumber));
				 ...
				 //$number = trim(preg_replace('/[0-9]/', '#', trim($this->_request->getParam('phonenumber'))));
	       		 ...
	       		 if (!in_array($number, $formats) && $format != 1)
				 {
			...
			/********* start validation of Email *******/ 
				...
				{
					if (!$validator->isValid(trim($this->_request->getParam('email'))) && $format != 1)
					{
					    $test_array = array (
		 ...
		 // HERE WE GET EMAIL NOTIFICATIONS FOR SENDING THE EMAIL, END HERE
		  ...
		  /* NOTE THESE RULES ARE HARD CODED IN THE ADMIN PANEL, IT WILL BE MANAGED BY THE ADMIN*/
                       ...
                       if(($start_time <= $customer_current_time)&& ($customer_current_time <= $end_time))
                       {
                             ...
                             $call_delay = $forms_data['call_delay_for_business_number'];
                       }else{
                               if($forms_data['home_phone_validated'] == 1)
                             		  $phone=$forms_data['home_phone'];
                               /*if(empty ($phone))
                               ...
                               $extension = $forms_data['home_phone_extension'];     
                                $call_delay = $forms_data['call_delay_for_home_number'];
                       }
				   //echo $phone.	'------1500';
		  }		  
			...
			if(trim($string_to_pass_to_twilio)!='')
				...
				$string_to_pass_to_twilio .=" To connect with the customer press the 1 key ";
                     ...
                     /* Start For randamly created caller_id according to doc bugs when selected caller_id as "Active Connection Number" updated by Hasan saeed*/    
                                   ...
                                   if(substr(trim($forms_data['flags']),1,1) == '1') 
                                   {
...
//                        die();
					...
					if(trim($emailnotification_data['send_email_notification_pros_leads'])=='yes' && trim($email) != '' && !empty($email))
		             {
                        $mail = new Zend_Mail();
                        	    ...
                        	    $fromEmail = trim($emailnotification_data['email_address_to_use_in_from_field_to_prospective_lead']);
                                ...
                                $mailBody = $customerHeader.$image."</div></div>";
                                $mail->setBodyHtml($mailBody);
                                $mail->setFrom($fromEmail, $form_owner_data['companyname']);
                        ...
                        {	
                        		...
                        		$image ="<br/><div style='padding-left: 0px;border: 0pt'><br/><a href='http://formactivate.com' target='_blank'><img src='http://formactivate.com/images/powered_by_formactivate.png' border='0' alt='FormActivate: Web Form Submission to Live Phone Contact in 20 Seconds' title='FormActivate: Web Form Submission to Live Phone Contact in 20 Seconds'></a></div>";
                        		$mail = new Zend_Mail(); 
                            	$mail->setFrom(SITE_SUPPORT_EMAIL, WEBSITE_NAME);    
                            	$mail->setSubject('A New FormActivate Inquiry');
                            	$Body.=" Thanks, <br><span style='color:black;'> ".WEBSITE_NAME."</span>".$image."</div></div>";
					...
					$dataOfPostCallDetail = substr($emailnotification_data['flags'],1,1)."-".trim($emailnotification_data['email_address_to_use_in_from_field_to_prospective_lead'])."-".$extension."-".$call_delay;
					
			if($connecttotwilio==1)
			{		
				$this->connecttotwilioAction(md5($time),$last_insert_id,$form_id,$user_phone,$notification_email,$authorizationkey,$format,$dataOfPostCallDetail);
 ...
 /** $this->connecttotwilioAction(md5($time),$last_insert_id,$form_id,$user_phone,$notification_email);
    ...
    public function connecttotwilioAction($ression ,$last_insert_id,$form_id,$user_phone,$email,$authorizationkey,$format,$dataOfPostCallDetail)
    {
         ...
         $dataOfPostCallDetail = $dataOfPostCallDetail;
         ...
         $notification_email = $post_call_detail_data[1];
          ...
          $call_delay = $post_call_detail_data[3];
         ...
         if($runtime_session->runtime_session==$rsession)
         {
                ...
                $caller_id = $formsTableData['caller_id'];
		...
		{
			$data=$inquiryTableData['announced_data']."@@@@".$formsTableData['to_repeat_the_announcement']."@@@@".$call_delay."@@@@".$extension; // to add by hasan ."%".$formsTableData['to_repeat_the_announcement']
