...
//            $Body.="<br><br> Thanks <br><span style='color:black;'> " . WEBSITE_NAME . "</span></div>";
    ...
    public function signupplanAction()
    {
        ...
        $this->view->data = $member_data;
        $this->view->card_num = $this->decrypt($member_data->card_num);
...
//      ini_set('display_errors', 'On');
        ...
        if ($id == $session->signup_session_id) {
            ...
            $this->view->card_type = $members_data['card_type'];
            $this->view->card_num = $this->decrypt($members_data['card_num']);
            $this->view->card_cvv = $members_data['card_cvv'];
            $this->view->expiry_month = $members_data['expiry_month'];
            $this->view->expiry_year = $members_data['expiry_year'];
            ...
            if ($this->_request->isPost()) {
                ...
                if ($members_data['braintree_customer_id'] == '') {
                    $result = Braintree_Customer::create(array(
                            ...
                            'creditCard' => array(
                                'number' => $this->decrypt($members_data['card_num']),
                                'expirationDate' => $members_data['expiry_month'] . '/' . $members_data['expiry_year'],
                                'cvv' => $members_data['card_cvv']
                            )
                        ));
        ...
        if ($this->_request->isPost()) {
            ...
            if (empty($email) || empty($pass)) {
                ...
                if (count($data) > 0) {
                    ...
                    //OLD B-4 : $this->_redirect('/customers/myaccount/id/' . $data['id']); //}
                    //$this->_redirect('forms/');
                } else {
                    $login_error = new Zend_Session_Namespace('Zend_Auth');
                    ...
                    if(!empty($members_data['braintree_token'])){
                        try {
                            $result = Braintree_CreditCard::update(
                              ...
                              array(
                                'number' => $this->_request->getPost('card_num'),
...
//                    $membersTable->update($members_data_braintree, 'id=' . $id);
        ...
        if (($id > 0) && !empty($id)) {
            ...
            if($members_data['plan_id'] != 5)
            {
                $this->view->card_num = "XXXXXXXXXXXX" . substr(htmlspecialchars(stripslashes($this->decrypt($members_data['card_num']))), -4);
                $this->view->card_num_org = $this->decrypt($members_data['card_num']);
            }
            $this->view->card_cvv = $members_data['card_cvv'];
            $this->view->expiry_month = $members_data['expiry_month'];
            $this->view->expiry_year = $members_data['expiry_year'];
...
?>
            <div id="formUserDetails" class="hr1b1" style="position: absolute; left: 37%;top:258px;">
                <img onclick="hideme('div_prorate')" src="<?= WEBSITE_URL ?>images/Close.png" style="float:right;">             <fieldset>
...
<?php } else { ?>
                            You have chosen to go from the $<?= number_format($subscriptions_org_data['price'], 2, '.', '') ?> plan to the $<?= number_format($subscriptions_new_data['price'], 2, '.', '') ?> Your plan will be updated immediately and you will be charged a prorated rate for the remainder of the current billing cycle. This amounts to $<?=
...
<?php
                ...
                <a href="<?php echo WEBSITE_URL.'customers/editpaymentdetails/id/'.$id; ?>">Pay Now</a>

                <br><br></span>
                    <input class="buttonsNext" type="button" value="Cancel"  onclick="hideme('div_prorate')" style="width:78px;"/>&nbsp;
...
<? for ($report_date = 0; $report_date < $thisusermonth; $report_date++) {
 ?>
                    <option value='fromdate/<?php echo date('m-d-Y', $start_date_value[$report_date]); ?>/todate/<?php echo date('m-d-Y', $end_date_value[$report_date]) ?>'  <?php if ('fromdate/' . date('m-d-Y', $start_date_value[$report_date]) . '/todate/' . date('m-d-Y', $end_date_value[$report_date]) == $selected_option) {
                        echo "selected";
