...
class FormsController extends BaseController
{
   ...
   public function init()
    ...
    public function validatePhoneNumberAction()
    {
        ...
        $client_number = $filter->filter($this->_request->getPost('number'));

        $number_type = $filter->filter($this->_request->getPost('number_type'));
...
//                     $session->home_phone_status = 0;
    ...
    public function opthrsAction()
    {
		...
		if(!empty($form_id))
		{
			$condition_customer = $opt_hrs->select()->where('customer_id='.$customer_id.' and form_id='.$form_id.' and status="1"')	->order('opt_hours.id asc');
		 ...
		 /* CHECKING CUSTOMER RULES FOR SENDING CALL END HERE */
			...
			$lastname=$this->_request->getPost('2');
		 	...
		 	if($this->_request->getPost('announce_1')=='1' && $this->_request->getPost('announce_2')=='1'){	
		 		...
		 		$string_to_pass_to_twilio .="Name is ".$firstname;
		 	}else if($this->_request->getPost('announce_2')=='1'){	 			
		 		$string_to_pass_to_twilio .="Name is ".$lastname;
		 	}
			...
			if($this->_request->getPost('announce_3')=='1'){	 	
				$string_to_pass_to_twilio .=" Email is ".$email;				
			}
			...
			if($this->_request->getPost('announce_5')=='1'){
					$string_to_pass_to_twilio .=" Company  is ".$company;
			}
			if($company!=''){	 					
			...
			if($this->_request->getPost('announce_6')=='1'){
				$string_to_pass_to_twilio .=" Street Address is ".$streetaddress;
			...
			if($this->_request->getPost('announce_7')=='1'){
				$string_to_pass_to_twilio .=" City is ".$city;
			...
			if($this->_request->getPost('announce_8')=='1'){
				$string_to_pass_to_twilio .=" State is ".$state;
			}
			...
			if($this->_request->getPost('announce_9')=='1'){
					$string_to_pass_to_twilio .=" Zip is ".$zip;
			}
			...
			if($this->_request->getPost('announce_10')=='1'){
				$string_to_pass_to_twilio .=" Country is ".$country;
			...
			if($this->_request->getPost('announce_4')=='1'){
				$string_to_pass_to_twilio .=" Phone Number is ".$homephone;
			...
			$string_to_pass_to_twilio .=" ".$forms_data['label']." is ".$custom1;
			...
			$forms_data=$form_std_fields->fetchRow($form_std_fields->select()->where('id='.$ann_cust2[1]));	
				$string_to_pass_to_twilio .=" ".$forms_data['label']." is ".$custom2;
			...
			$forms_data=$form_std_fields->fetchRow($form_std_fields->select()->where('id='.$ann_cust3[1]));	
				$string_to_pass_to_twilio .=" ".$forms_data['label']." is ".$custom3;
			...
			$forms_data=$form_std_fields->fetchRow($form_std_fields->select()->where('id='.$ann_cust4[1]));	
					$string_to_pass_to_twilio .=" ".$forms_data['label']." is ".$custom4;
			...
			$forms_data=$form_std_fields->fetchRow($form_std_fields->select()->where('id='.$ann_cust5[1]));	
				$string_to_pass_to_twilio .=" ".$forms_data['label']." is ".$custom5;
			...
			/* Instantiate a new Twilio Rest Client */
               ...
               $response = $client->request("/$ApiVersion/Accounts/$AccountSid/Calls",
                    "POST", array(
                        ...
                        "StatusCallback" => WEBSITE_URL."twilios/twilio_call_time_update.php?notification_email={$notification_email}"
                    ));
...
//			$this->_redirect($redirect_url);  exit;
			...
			$this->_redirect('/forms/thanks');   exit;
                        ...
                        $plan_mail_body .= 'http://app.formactivate.com/customers/editplandetails/id/'.$customer_id . "<br /><br />";
                        ...
                        but you will no longer receive any phone calls for new leads. <br />
                        If you would like to change your plan, please do so by clicking on the following link:<br />";
                        $plan_mail_body2 .= 'http://app.formactivate.com/customers/editplandetails/id/'.$customer_id . "<br /><br />";
                        ...
                        if($plan_id == 2 && $total_remaining_calls == 5)
                        {
                            $result=$mail->send(); $result2=$mail2->send();
                        }
                        else if($plan_id == 3 && $total_remaining_calls == 20)
                        {
                            $result=$mail->send(); $result2=$mail2->send();
                        }
                        else if($plan_id == 4 && $total_remaining_calls == 50)
                        {
                            $result=$mail->send(); $result2=$mail2->send();
                        }
